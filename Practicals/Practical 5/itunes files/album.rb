...
class Album
  ...
  def self.build_all(data, albums = [])
    ...
    album_names.uniq.each { |album_name| albums << build_an_album_called(data, album_name) }
    #p albums
    albums
  end
  ...
  def self.build_an_album_called(data, album_name)
    ...
    data.songs.each do |song|
      if song.album == album_name
        ...
        # e.g. Greatest Hits 1990s will have "Wonderwall" by Oasis, "Give It Away" by The Red Hot Chili Peppers....
        owners.clean_up.each.include?(song.owners) ? owners : owners << song.owners
      end
    end
